...
app.get("/reviews", function(req,res){
    model.Review.find().sort({ dateCreated: '-1'}).then(function(reviews){
        for (var i = 0; i < reviews.length; i++){
            var dateReviewCreated = new Date(reviews[i].date);
            reviews[i].date = moment(dateReviewCreated).fromNow();
